...
const chatController = restController.chatController;

export function* previewSaga() {
  try {
    const { data } = yield chatController.getPreviewChat();
...
export function* getDialog(action) {
  try {
    const { data } = yield chatController.getDialog(action.data);
...
export function* sendMessage(action) {
  try {
    const { data } = yield chatController.newMessage(action.data);
...
export function* changeChatFavorite(action) {
  try {
    const { data } = yield chatController.changeChatFavorite(action.data);
    const { messagesPreview } = yield select((state) => state.chatStore);
    messagesPreview.forEach((preview) => {
...
export function* changeChatBlock(action) {
  try {
    const { data } = yield chatController.changeChatBlock(action.data);
    const { messagesPreview } = yield select((state) => state.chatStore);
    messagesPreview.forEach((preview) => {
...
export function* getCatalogListSaga(action) {
  try {
    const { data } = yield chatController.getCatalogList(action.data);
...
export function* addChatToCatalog(action) {
  try {
    const { data } = yield chatController.addChatToCatalog(action.data);
...
export function* createCatalog(action) {
  try {
    const { data } = yield chatController.createCatalog(action.data);
...
export function* deleteCatalog(action) {
  try {
    yield chatController.deleteCatalog(action.data);
    const { catalogList } = yield select((state) => state.chatStore);
...
export function* removeChatFromCatalogSaga(action) {
  try {
    const { data } = yield chatController.removeChatFromCatalog(action.data);
    const { catalogList } = yield select((state) => state.chatStore);
    for (let i = 0; i < catalogList.length; i++) {
      if (catalogList[i]._id === data._id) {
        catalogList[i].chats = data.chats;
...
export function* changeCatalogName(action) {
  try {
    const { data } = yield chatController.changeCatalogName(action.data);
    const { catalogList } = yield select((state) => state.chatStore);
    for (let i = 0; i < catalogList.length; i++) {
      if (catalogList[i]._id === data._id) {
